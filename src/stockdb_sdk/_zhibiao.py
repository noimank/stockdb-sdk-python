...
from ._sdk import get_default_client, get_default_raw_rd
...
def jisuan(
    ...
    *,
    ...
    支持 MACD、KDJ、RSI、BOLL 等常用指标，以及 MA/EMA 等基础指标，
    并可附加金叉/死叉（``cross``）信号。
    ...
    Args:
        name: 指标名，支持逗号分隔或列表，如 ``"macd"``、``"macd,kdj"``、
            ...
            bbi/dmi/taq/ktn/trix/vr/cr/emv/dpo/brar/dfma/mtm/mass/roc/expma/
            obv/mfi/asi/xsii``；特殊指标 ``"zhishu"`` 用于合成指数。
        codes: 股票代码或代码列表。
        start: 起始日期 ``"YYYYMMDD"``，默认 ``"20260302"``。
        end: 结束日期 ``"YYYYMMDD"``。
        frequency: 周期，``"day"``/``"d"``（日）、``"min"``/``"1m"``（分钟）。
        method: 指数合成权重方式（仅 ``name="zhishu"`` 时生效）：
            ``1`` 等权、``2`` 流通市值、``3`` 成交额、``4`` 成交量、``5`` 总市值。
        base: 指数基点（仅 ``name="zhishu"`` 时生效），默认 ``1000.0``。
        fq: 复权方式，``"qfq"``（默认）/``"hfq"``/``None``。
        fields: 基础指标的输入字段（如 ``"close"``/``"open"``），
            仅对 ma/ema 等基础指标生效；多个字段会分别计算并合并。
        n: 指标参数。单个指标传整数（如 ``n=5``）；多个指标时传与
            ``name`` 等长的列表。
        cross: 是否计算交叉信号。``False`` 不计算；``True`` 计算金叉/死叉；
            ``"with_value"`` 同时保留指标数值与信号。
    ...
    Example::
        ...
        # MACD 指标
        r = sdk.jisuan("macd", "000001.SZ", start="20260101", end="20260131")

        # MA5 / MA10，并输出金叉信号
        r = sdk.jisuan("ma", "000001.SZ", n=[5, 10], cross=True)

        # 多股票 KDJ
        r = sdk.jisuan("kdj", ["000001.SZ", "600000.SH"], start="20260101")
    """
