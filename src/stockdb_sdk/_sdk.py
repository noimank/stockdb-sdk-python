...
def init(host: str = "127.0.0.1", port: int = 7899,
    ...
    配置完成后，``gp`` / ``rd`` / ``get_data`` 等全局入口即指向该服务端。
    ...
    Example::
        ...
        bars = sdk.get_data("000001.SZ", start="20260101")
    """
...
class StockDBClient:
    ...
    提供统一的 :meth:`get_data` 接口，支持同步/异步操作，并支持在内存中
    合成周 K、月 K 及多分钟 K 线。
    ...
    def __init__(self, host: Optional[str] = None, port: Optional[int] = None,
        ...
        若 ``host`` 等参数未显式给出，则回退到 :func:`init` 配置的默认值。
        实例化时会一次性预加载全部复权因子到内存，用于后续复权折算。
        ...
        Example::
            ...
            bars = client.get_data("000001.SZ", start="20260101")
        """
        try:
    ...
    def get_data(
        ...
        Args:
            code: 股票代码，如 ``"000001.SZ"``；也支持列表批量查询
                （如 ``["000001.SZ", "600000.SH"]``）。
            start: 起始日期，``"YYYYMMDD"`` 格式，含当日。
            end: 结束日期，``"YYYYMMDD"`` 格式，含当日。
            frequency: 周期。日 K ``"1d"``；分钟 K ``"1m"/"5m"/"15m"/"30m"/"60m"``；
                周 K ``"1w"``；月 K ``"1M"``。
            fields: 需要返回的字段，逗号分隔字符串或列表，如 ``"date,open,close"``。
                不传则返回全部字段。
            limit: 最多返回的记录条数。
            desc: 是否按时间降序返回。
            as_df: 为 ``True`` 时返回 ``pandas.DataFrame``（需安装 pandas）。
            fq: 复权方式。``"qfq"`` 前复权（默认）、``"hfq"`` 后复权、
                ``None`` 不复权。

        Returns:
            单股票时返回 ``List[Dict]``；批量时返回 ``Dict[code, List[Dict]]``；
            ``as_df=True`` 时返回 ``DataFrame``。

        Example::

            # 单只股票日 K，前复权，指定字段
            bars = client.get_data("000001.SZ", start="20260101", end="20260131",
                                   frequency="1d", fields="date,open,close", fq="qfq")

            # 多股票批量，返回 DataFrame
            df = client.get_data(["000001.SZ", "600000.SH"], start="20260101",
                                 as_df=True)

            # 周 K 线
            weekly = client.get_data("000001.SZ", frequency="1w")
        """
...
def warm_default_connection() -> StockDBClient:
    ...
    from . import _zhibiao
    _zhibiao.reset_default_connection()
    _zhibiao.warm_default_connection()
    return client
